...
class Converter8(object):
    ...
    def contract(self,vertex, edge, num, merge, k):
        ...
        if num == 2:
            while (len(vertex) > math.ceil(2 * k)):
                ...
                for e in edge:
                    all_weight_sum += e[2]
                ind = random.randrange(1, all_weight_sum+1)
                for e in edge:
                    temp += e[2]
                    if ind<temp:
                        isit_index = edge.index(e)
                u = edge[isit_index][0]
                v = edge[isit_index][1]
                vertex.remove(v)
    ...
    def find_cut(self, vertex_list,group):
            ...
            if vertex in self.connect_subnets_temp:
                for connect_vertex in self.connect_subnets_temp[vertex]:
                    if [vertex_to_number[connect_vertex], vertex_to_number[vertex]] not in edges:
                        ...
                        edges.append([vertex_to_number[vertex], vertex_to_number[connect_vertex], 1])
                    else:
                        G.add_edge(vertex_to_number[connect_vertex], vertex_to_number[vertex], **{'weight': 2})
                        edges[edges.index([vertex_to_number[connect_vertex], vertex_to_number[vertex], 1])] = [
                            vertex_to_number[connect_vertex], vertex_to_number[vertex], 2]
        ...
        for i in range(0,len(minimal_cut)):
            ...
            if (sum-0.5*minimal_cut[i])/(0.5*minimal_cut[i]) > max_average:
                max_average = (sum-0.5*minimal_cut[i])/(0.5*minimal_cut[i])
                max_acl_save = sum-0.5*minimal_cut[i]
                index = i
                    ...
                    for subnet in self.distribution[group]:
                        if subnet in self.connect_subnets:
                            for connect in self.connect_subnets[subnet]:
                                if connect in self.distribution[divided_group[initial][other_group_index]] and [
                                    divided_group[initial][other_group_index], group] not in already_count:
                                    peering_now += 1
                                    already_peering = 1
