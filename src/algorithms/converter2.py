...
class Converter2(object):
    ...
    def contract1(self,s_t,g,in_set,vertex_num):
    ...
    def sw(self,vertex_list,two_nums,merge,g):
        ...
        for i in range(1,vertex_num):
            s_t = [0, 0]
            ans = self.contract1(s_t,g,in_set,vertex_num)
            in_set[s_t[1]] = 1
            ...
            for edge in G.edges():
                if partition[edge[0]] != partition[edge[1]]:
                    if partition[edge[0]] not in community_cut:
                        community_cut[partition[edge[0]]] = 1
                    else:
                        community_cut[partition[edge[0]]] += 1
                    if partition[edge[1]] not in community_cut:
                        community_cut[partition[edge[1]]] = 1
                    else:
                        community_cut[partition[edge[1]]] += 1
            if len(community_cut)!=0:
                initial_average_delay = 150
                initial_acl_nums = vertex_num * (vertex_num - 1)
                connect_pair = 0
                for vertex in vertex_list:
                    if vertex in self.connect_subnets_temp:
                        initial_acl_nums -= len(self.connect_subnets_temp[vertex])
                        connect_pair += len(self.connect_subnets_temp[vertex])
                connect_pair = connect_pair / 2
                if initial_acl_nums == 0:
                ...
                for community in community_cut:
                    if (community_nodenum[community] * (vertex_num - community_nodenum[community]) - community_cut[
                        community]) / \
                            community_cut[community] > max_average:
                        max_average = (community_nodenum[community] * (vertex_num - community_nodenum[community]) -
                                       community_cut[community]) / community_cut[community]
                        max_acl_save = community_nodenum[community] * (vertex_num - community_nodenum[community]) - \
                                       community_cut[community]
                        max_community = community
                save_acl_1 = 2 * (community_nodenum[max_community] * (vertex_num - community_nodenum[max_community]) -
                                  community_cut[max_community])
                add_delay_1 = ((connect_pair - community_cut[max_community]) * 150 + community_cut[
                    max_community] * 330) / connect_pair - 150

                two_nums = [0, 0]
                min_cut = self.sw(vertex_list, two_nums, merge, g)
                save_acl_2 = 2 * (two_nums[0] * two_nums[1] - min_cut)
                add_delay_2 = ((connect_pair - min_cut) * 150 + min_cut * 330) / connect_pair - 150

                if ((save_acl_1 - save_acl_2) == 0 and (add_delay_1 - add_delay_2) <= 0) or \
                        (((save_acl_1 - save_acl_2) > 0 and (add_delay_1 - add_delay_2) > 0) and \
                         ((save_acl_1 - save_acl_2) / initial_acl_nums) / (
                                 (add_delay_1 - add_delay_2) / initial_average_delay) > 2):
                    self.group_divide[group].clear()
                    for node in partition:
    ...
    def reduce_acls(self):
        ...
        while True:
            ...
            if over_the_route_num ==1 or (peering_sum-initial_group_peering[initial]+peering_now) > 5 :
                for subnet in self.distribution[group_to_add]:
                    self.distribution[group_to_divide].append(subnet)
