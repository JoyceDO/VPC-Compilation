...
import community2 as community_louvain
import matplotlib.cm as cm

class Converter6(object):
    def __init__(self):

        # 计算意图网中每个子网的所有可达子网，key:子网id，value:[子网id, ]
        self.connect_subnets = {}

        self.connect_subnets_undirect = {}
    ...
    def dfs(self):
        ...
        for subnet in self.connect_subnets_undirect:
            if subnet not in visited:
                ...
                while stack:
                    node = stack.pop()
                    cc[node] = self.connect_subnets_undirect
                    for neighbour in self.connect_subnets_undirect[node]:
    ...
    def contract(self,vertex, edge, num, merge, k):
        if num == 2:
            while (len(vertex) > math.ceil(2 * k)):
                if len(edge) == 0:
                    return len(edge)
            ...
            left_has_vertex = 0
            right_has_vertex = 0
            left_origin = right_origin = 0
            vertex_copy = copy.deepcopy(vertex)

            for i in range(0, len(vertex_copy)):
                if left_has_vertex == 0 and distribution[i] == 0:
                    left_has_vertex = 1
                    left_origin = vertex_copy[i]
                    continue
                if right_has_vertex == 0 and distribution[i] == 1:
                    right_has_vertex = 1
                    right_origin = vertex_copy[i]
                    continue
                delete_edge = []
                if distribution[i] == 0:
                    for j in range(0, len(edge)):
                        if edge[j][0] == vertex_copy[i]:
                            edge[j][0] = left_origin
                        elif edge[j][1] == vertex_copy[i]:
                            edge[j][1] = left_origin
                        if edge[j][0] == edge[j][1]:
                            delete_edge.append(edge[j])
                    for edgee in delete_edge:
                        edge.remove(edgee)
                    del delete_edge
                    for merge_v in merge[vertex_copy[i]]:
                        merge[left_origin].append(merge_v)
                    del merge[vertex_copy[i]]
                    vertex.remove(vertex_copy[i])
                if distribution[i] == 1:
                    for j in range(0, len(edge)):
                        if edge[j][0] == vertex_copy[i]:
                            edge[j][0] = right_origin
                        elif edge[j][1] == vertex_copy[i]:
                            edge[j][1] = right_origin
                        if edge[j][0] == edge[j][1]:
                            delete_edge.append(edge[j])
                    for edgee in delete_edge:
                        edge.remove(edgee)
                    del delete_edge
                    for merge_v in merge[vertex_copy[i]]:
                        merge[right_origin].append(merge_v)
                    del merge[vertex_copy[i]]
                    vertex.remove(vertex_copy[i])
    ...
    def recursive_contract(self,vertex, edge, vertex_num, minimum_cut, previous_merge, k):
        ...
        while (sub_size > math.ceil(6 * k)and j <11):
            #print(j)
            j = j+1
            sub_size = math.ceil(1+sub_size / math.pow(2, 1 / (2 * k)))
    ...
    def find_cut(self, vertex_list,group):
        ...
        for vertex in vertex_list:
            for connect_vertex in self.connect_subnets_temp[vertex]:
                if [vertex_to_number[connect_vertex],vertex_to_number[vertex]] not in edges:
                    ...
                    G.add_edge(vertex_to_number[vertex], vertex_to_number[connect_vertex], **{'weight': 1})
                else:
                    G.add_edge(vertex_to_number[connect_vertex], vertex_to_number[vertex], **{'weight': 2})
        ...
        for node in partition:
            if partition[node] not in community_nodenum:
                community_nodenum[partition[node]] = 1
                community_num += 1
            else:
                community_nodenum[partition[node]] += 1
        ...
        for edge in G.edges():
            if partition[edge[0]] != partition[edge[1]]:
                if partition[edge[0]] not in community_cut:
                    community_cut[partition[edge[0]]] = G.get_edge_data(edge[0], edge[1]).get('weight')
                else:
                    community_cut[partition[edge[0]]] += G.get_edge_data(edge[0], edge[1]).get('weight')
                if partition[edge[1]] not in community_cut:
                    community_cut[partition[edge[1]]] = G.get_edge_data(edge[0], edge[1]).get('weight')
                else:
                    community_cut[partition[edge[1]]] += G.get_edge_data(edge[0], edge[1]).get('weight')
        ...
        for community in community_cut:

            if (community_nodenum[community] * (vertex_num - community_nodenum[community]) - 0.5*community_cut[community]) / \
                    (0.5*community_cut[community]) > max_average:
                max_average = (community_nodenum[community] * (vertex_num - community_nodenum[community]) -
                               0.5*community_cut[community]) / (0.5*community_cut[community])
                max_acl_save = community_nodenum[community] * (vertex_num - community_nodenum[community]) - \
                               0.5*community_cut[community]
                max_community = community
        self.group_divide[group].clear()
        for node in partition:
            if partition[node] == max_community:
                self.group_divide[group].append(number_to_vertex[node])
        return max_acl_save
    ...
    def reduce_acls(self):
        ...
        while True:
            ...
            if (len(self.group_divide[group_to_divide]) == 0):
                new_groups.clear()
            ...
            for group in divided_group[initial]:
                for other_group_index in range(0,length):
                    already_peering =0
                    for subnet in self.distribution[group]:
                        for connect in self.connect_subnets[subnet]:
                            if connect in self.distribution[divided_group[initial][other_group_index]] and [divided_group[initial][other_group_index],group] not in already_count:
                                peering_now += 1
                                already_peering=1
                                already_count.append([group,divided_group[initial][other_group_index]])
                                break
                        if already_peering:
                            break
            already_count.clear()
            ...
            if over_the_route_num ==1 or (peering_sum-initial_group_peering[initial]+peering_now) > 50  :
                if over_the_route_num == 1:
                    print("-----------------------")
                for subnet in self.distribution[group_to_add]:
                    self.distribution[group_to_divide].append(subnet)
    ...
    def convert_intent_network(self,int_nw,G,intsubnetid_to_subnet,pos):
        ...
        for reachability in int_nw.intents_r.values():
            ...
            if subnet2_id in self.isolationists:
                self.isolationists.remove(subnet2_id)
            if subnet1_id in self.isolationists:
                self.isolationists.remove(subnet1_id)

            if subnet1_id not in self.connect_subnets_undirect:
                self.connect_subnets_undirect[subnet1_id] = []
            if subnet2_id not in self.connect_subnets_undirect[subnet1_id]:
                self.connect_subnets_undirect[subnet1_id].append(subnet2_id)

            if subnet2_id not in self.connect_subnets_undirect:
                self.connect_subnets_undirect[subnet2_id] = []
            if subnet1_id not in self.connect_subnets_undirect[subnet2_id]:
                self.connect_subnets_undirect[subnet2_id].append(subnet1_id)
