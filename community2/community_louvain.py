...
def partition_at_level(dendrogram, level, graph):#, pos):
...
def best_partition(graph,
    ...
    dendo = generate_dendrogram(graph,
                                ...
                                randomize,
                                random_state)
    return partition_at_level(dendo, len(dendo) - 1, graph)#, pos)
...
def generate_dendrogram(graph,
    ...
    node_sum = len(graph.nodes())
    node_node_num = dict([])
    pos = nx.spring_layout(graph)

    current_graph = graph.copy()
    status = Status()
    status.init(current_graph, weight, node_node_num, part_init)
    status_list = list()
    __one_level(current_graph, status, weight, resolution, random_state, node_sum, node_node_num)
    ...
    #mod = new_mod
    current_graph = induced_graph(partition, current_graph, node_node_num, weight)
    status.init(current_graph, weight, node_node_num)

    while True:
        count = __one_level(current_graph, status, weight, resolution, random_state, node_sum, node_node_num)
        if count == 1:
            break
        partition = __renumber(status.node2com)
...
def induced_graph(partition, graph, node_node_num, weight="weight"):
    ...
    temp = copy.deepcopy(node_node_num)
    node_node_num.clear()
    for node in partition:
        node_node_num[partition[node]] = node_node_num.get(partition[node],0) + temp[node]
...
def __one_level(graph, status, weight_key, resolution, random_state, node_sum, node_node_num):
    ...
    while modified and nb_pass_done != __PASS_MAX:
        ...
        for node in __randomize(graph.nodes(), random_state):
            ...
            if status.degrees[com_node] - 2 * status.internals[com_node] == 0:
                continue

            neigh_communities = __neighcom(node, graph, status, weight_key)

            origin_average1 = (status.com_nodes[com_node]*(node_sum - status.com_nodes[com_node]) - 0.5*(status.degrees[com_node]-2*status.internals[com_node]))/(0.5*(status.degrees[com_node]-2*status.internals[com_node]))
            __remove(node, com_node,
                     neigh_communities.get(com_node, 0.), status, node_node_num)
            best_com = com_node
            best_increase = 0
            for com, dnc in __randomize(neigh_communities.items(), random_state):

                origin_average2 = (status.com_nodes[com]*(node_sum - status.com_nodes[com]) - 0.5*(status.degrees[com]-2*status.internals[com]))/(0.5*(status.degrees[com]-2*status.internals[com]))
                if ((status.degrees[com]+status.gdegrees.get(node,0.))-2*(status.internals[com]+status.loops.get(node,0.)+neigh_communities.get(com, 0.))) == 0:
                    continue
                present_average = ((status.com_nodes[com]+node_node_num[node])*(node_sum - status.com_nodes[com]-node_node_num[node]) - 0.5*((status.degrees[com]+status.gdegrees.get(node,0.))-2*(status.internals[com]+status.loops.get(node,0.)+neigh_communities.get(com, 0.))))/(0.5*((status.degrees[com]+status.gdegrees.get(node,0.))-2*(status.internals[com]+status.loops.get(node,0.)+neigh_communities.get(com, 0.))))
                if present_average> origin_average1 and present_average>origin_average2 and present_average > best_increase:
                    best_increase = present_average
                    best_com = com
            __insert(node, best_com,
                     neigh_communities.get(best_com, 0.), status, node_node_num)
...
def __remove(node, com, weight, status, node_node_num):
    ...
    status.com_nodes[com] -= node_node_num[node]


def __insert(node, com, weight, status, node_node_num):
